...
class Manager:
    ...
    def manual_control(self, f, body):
        ...
        hsbk = lifx.rgb_to_hsbk(
            int(body.get("r", 0)), int(body.get("g", 0)), int(body.get("b", 0)),
            intensity, self.settings["kelvin"])
...
async function control(id){
  ...
  updateSquares(id);
  const j=await api('/api/fixture/control',{id,r,g,b,
...
function renderHelp(){
  ...
  <div class="card help">
    ...
    <ol>
      ...
      <li><b>Patch in QLab.</b> Add one <b>8-channel</b> fixture per bulb at the
        ...
        <b>R, G, B, Intensity, Mode, Speed, Strobe, FX</b> — drive raw RGB (no
        RGBA/RGBW colour parameter), with Intensity on channel 4.</li>
  ...
  <div class="card help">
    ...
    <ol>
      <li><b>Red</b> · <b>Green</b> · <b>Blue</b> (ch 1–3) — the colour (0–255 each).</li>
      <li><b>Intensity</b> (ch 4) — overall brightness / master dimmer.</li>
      <li><b>Mode</b> (ch 5) — selects a static look or an effect (see the table below).</li>
      <li><b>Speed</b> (ch 6) — effect rate, slow → fast.</li>
      <li><b>Strobe</b> (ch 7) — strobe overlay; 0 = off. Takes priority over the mode.</li>
      <li><b>FX Scene</b> (ch 8) — triggers a Candle FX scene (see below); 0 = off.</li>
    </ol>
  </div>

  <div class="card help">
    <h2>Effect / Mode channel (channel 5)</h2>
    ...
    <p class=muted style="margin-top:10px">Speed = channel 6 (slow→fast). Strobe =
    channel 7 (0 = off) and overrides the mode.</p>
  ...
  <div class="card help">
    <h2>FX Scene channel (channel 8)</h2>
    <p>Set channel 8 to one of these values to trigger a Candle FX scene.
    Morph/Flame/Sky run on the bulb firmware; Flicker/Twinkle/Meteor are
    generated by the bridge.</p>
    <div class=tablewrap><table><tr><th>Set ch8 to</th><th>Scene</th><th>Runs on</th></tr>
    ...
    <p class=muted style="margin-top:10px">Strobe (ch7) overrides FX; FX overrides the Mode channel.
    Speed (ch6) sets the FX rate. Each scene has a &plusmn;9 band around its value.</p>
  </div>
